...
extension Rotatable where Self: CropperViewController {
    ...
    public func rotate90degrees(clockwise: Bool = true) {
        ...
        let dxScale: CGFloat = clockwise ? (-animationContainer.width * scale * 3) : (animationContainer.width)
        let dyScale: CGFloat = clockwise ? (-animationContainer.height * scale * 3) : (animationContainer.height)
        let frame = animationContainer.bounds.insetBy(dx: dxScale, dy: dyScale)
