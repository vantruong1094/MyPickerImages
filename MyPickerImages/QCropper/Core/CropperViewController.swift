...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    var isCircular: Bool
    var optionStype: OptionScreen = .def
    ...
    lazy var topInset: CGFloat = UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.safeAreaInsets.top ?? 0
    lazy var bottomInset: CGFloat = UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.safeAreaInsets.bottom ?? 0
    ...
    lazy var titleEditImageLabel: UILabel = {
       let lbl = UILabel()
        lbl.text = "画像の編集"
        lbl.textColor = .white
...
// MARK: - Private Methods
    ...
    open func resetToDefaultLayout() {
        ...
        if optionStype == .thumbnailVideo {
            toolbar.size.height += 100
        }
        ...
        var topHeight = topBar.isHidden ? view.safeAreaInsets.top : topBar.height
        let toolbarHeight = toolbar.isHidden ? view.safeAreaInsets.bottom : toolbar.height
        var bottomHeight = (angleRuler.isHidden && aspectRatioPicker.isHidden) ? toolbarHeight : bottomView.height
        
        if optionStype == .thumbnailVideo {
            topHeight = topInset + 114 * view.frame.height / 812
            bottomHeight = bottomInset + 124 * view.frame.height / 812
        }
